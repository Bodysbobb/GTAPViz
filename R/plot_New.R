...
#' @param var_name_by_description If TRUE, uses "Description" column for titles instead of "Variable".
...
#' @param var_name_by_description Logical. If TRUE, uses the "Description" column from plot_var as the plot title.
...
#' @param width_dodge Numeric. Dodge width for bar position adjustment. Default is 0.5.
...
#' @param title_suffix Optional character string to append to the title.
#' @param compare_by_experiment Logical. If `TRUE`, compares experiments within x-axis categories.
#' @param var_name_by_description Logical. If TRUE, uses the "Description" column for plot titles instead
...
macro_plot <- function(data, plot_var = NULL,
  ...
  if (!is.null(plot_var)) {
    ...
    if (var_name_by_description && !"Description" %in% names(plot_var))
      stop('plot_var data frame must contain "Description" column when var_name_by_description=TRUE')

    if (!var_name_by_description && !"PlotTitle" %in% names(plot_var) && !"Description" %in% names(plot_var))
      stop('plot_var data frame must contain either "PlotTitle" or "Description" column')

    data <- data[data$Variable %in% plot_var$Variable, ]
    if (nrow(data) == 0) stop("No matching variables found in the data")

    title_col <- if (var_name_by_description && "Description" %in% names(plot_var)) {
      "Description"
    } else if (!var_name_by_description && "PlotTitle" %in% names(plot_var)) {
      "PlotTitle"
...
#' @param var_name_by_description Logical. If `TRUE`, uses the "Description" column as the title instead of "Variable". Default is `TRUE`.
#'
#' @return A list of ggplot objects or a single ggplot object depending on settings
#' @author Pattawee Puangchit
#' @export
