...
#' @description Processes GTAP data from SL4 and HAR files with comprehensive support for dynamic analysis,
#' regional aggregation, and deviation calculations across experiments.
#'
#' @details This function extends the base GTAP data processing workflow with additional capabilities
...
#' @param sl4_suffix Character. Custom suffix for SL4 files.
#' @param har_suffix Character. Custom suffix for HAR files.
...
#' @param sl4_mapping_info Data frame or NULL. Mapping information for SL4 variables.
#' @param har_mapping_info Data frame or NULL. Mapping information for HAR variables.
#' @param sl4_extract_method Character. SL4 extraction method.
#' @param har_extract_method Character. HAR extraction method.
...
#' @param subtotal_level Logical. If TRUE, includes subtotal data.
#' @param plot_data Logical. If TRUE, prepares data for plotting and assigns to variables.
#' @param output_formats Character vector or list. Exports data in these formats.
#' @param process_macro Logical. If TRUE, processes macro variables from SL4 files.
#' @param sl4_output_name Character. Variable name for SL4 plotting data.
#' @param har_output_name Character. Variable name for HAR plotting data.
#' @param macro_output_name Character. Variable name for GTAP macro data.
#' @param mapping_input Data frame. Maps experiment IDs to cases and periods.
#' @param agg_mapping List. Mapping for regional or sectoral aggregation.
#' @param aggregate Logical. If TRUE, performs regional/sectoral aggregation.
#' @param add_world Logical. If TRUE, adds "World" totals when aggregating regions.
#' @param cal_deviation Logical. If TRUE, calculates deviations between scenarios.
#' @param base_var Character. Case identifier for baseline scenario.
#' @param policy_var Character. Case identifier for policy scenario.
#' @param calculation_agg Character. Operation for aggregation.
#' @param calculation_dev Character. Operation for deviation calculation.
#'
#' @return A list containing the processed datasets with enhanced information for dynamic analysis.
#' @export
...
#' @description Creates a mapping data frame of simulation experiments to cases and periods
#' for dynamic GE model analysis.
#'
#' @param type Naming pattern
#' @param base Identifiers for baseline cases
#' @param pol Identifiers for policy cases
#' @param base_rerun Identifiers for baseline rerun cases
#' @param other Identifiers for other case types
#' @param pattern Pattern to be combined with case identifiers
#' @param increment Step size for numeric sequences
#' @param separator Separator between pattern and case identifier
#' @param period_pattern Creates period ranges between patterns
#' @param period_prefix Prefix for period names
#' @param output Name for result variable
#' @return A data frame mapping experiments to cases and periods
#'
#' @return A data frame mapping experiments to cases and periods.
#' @export
#' @author Pattawee Puangchit
...
#' @description Adds Case and Period information to GTAP data based on a provided mapping.
#'
#' @param data_list A data frame or list containing GTAP data structures.
#' @param external_map Data frame with mapping information. Must contain columns:
#'        "Input", "Case", and "Period".
#'
#' @return The input data structure with added Case and Period columns.
#' @keywords internal
#' @author Pattawee Puangchit
...
#' @title Calculate Deviations Between Policy and Base Scenarios
#' @description Computes deviations between policy and baseline scenarios in GTAP data.
#'
...
#' @title Create Aggregation Mapping Structure
#' @description Creates a hierarchical mapping structure for region/sector aggregation.
#'
#' @param mapping_file A data frame or path to a file with mapping information.
#' @param dimension_col Character. Name of column with dimension information.
#' @param item_col Character. Name of column with original GTAP names.
#' @param group_col Character. Name of column with aggregation group names.
#'
#' @return A nested list with aggregation mappings.
#' @keywords internal
#' @author Pattawee Puangchit
  ...
  if (is.character(mapping_file)) {
    ...
    if (file.exists(mapping_file)) {
      if (grepl("\\.xlsx$|\\.xls$", mapping_file)) {
        ...
        mapping_data <- utils::read.csv(mapping_file, stringsAsFactors = FALSE)
      } else {
...
#' @title Aggregate Regions and Sectors in GTAP Data
#' @description Aggregates regions, sectors, or other dimensions in GTAP data.
#'
#' @param data_list A data frame or list containing GTAP data to be aggregated.
#' @param agg_mapping A list containing aggregation mappings.
#' @param calculation Character. Operation for aggregation.
#' @param add_world Logical. If TRUE, adds a "World" aggregate for regions.
#'
