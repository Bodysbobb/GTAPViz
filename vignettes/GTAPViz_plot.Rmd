...
mapping.output <- paste0(project.folder, "/map/OutputMapping.xlsx")
...
The visualization framework is based on the **`sl4map`** and **`harmap`** sheets, which define the variables and figure titles used for plotting. Users can specify these directly within an external mapping file or generate them dynamically in the R environment.
...
The `<experiment>` variable in [setting](#sec:setting) contains experiment names from which results will be extracted and merged into a single output for further analysis, such as graph creation. Multiple experiments can be included, but increasing their number will extend processing time.

## Input Files Format

All input files must be stored in `<input.folder>` (default: `"in"` inside `<project.folder>`). File names must follow `<experiment>` to ensure consistency between input each `.sl4` file must have a corresponding `-WEL.HAR` file
...
All figures and tables will be sorted according to the order of your input in `<experiment>`, `<selected_regions>`, and `<selected_sector>`.  
For example:  
...
report_table(region.dta, vars = sl4map$Variable, 
             col_names = sl4map$Description,  
             x_axis_from = "Region",
             output_dir = output.folder)
```
...
If you need to automatically modify column names across all datasets and/or rename the data list names simultaneously—such as changing the column name `REG` to `Region` or `COMM` to `Commodity`—the following command will be useful:
